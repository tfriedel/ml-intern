...
_LOCAL_FIRST_PRELUDE = (
    ...
    "  - For research / parallel sub-investigations, use the built-in\n"
    "    `Task` tool with `subagent_type=\"ml-research\"` (defined in\n"
    "    `.claude/agents/ml-research.md`). It's restricted to the\n"
    "    read-only HF research toolset (hf_papers, explore/fetch_hf_docs,\n"
    "    github_{find_examples,list_repos,read_file}, hf_inspect_dataset,\n"
    "    hf_repo_files) and carries the literature-first system prompt.\n"
    "    Be specific in the prompt — name anchor papers or arxiv IDs\n"
    "    when you have them. The legacy `research` tool is NOT available\n"
    "    on this backend. A bare `Task(...)` without `subagent_type` gets\n"
    "    a generic sub-agent and is rarely what you want for ML research.\n"
